 ...
 * (identity-service/env/development.secret.env · JWT_PRIVATE_KEY_PEM) que el public-bff valida con su pública.
...
const SECRET = new URL('../../services/identity-service/env/development.secret.env', import.meta.url);
...
if (!m) {
  console.error('mint-token: no encontré JWT_PRIVATE_KEY_PEM en development.secret.env');
  process.exit(1);
}
const pem = m[1].trim();
