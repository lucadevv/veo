...
export async function resolveEffectiveOffering(
  ...
  // develop expone `mode` (el modo EFECTIVO ya resuelto) en ResolvedOffering; el único consumidor de esta fn
  // (waypoint-proposal) usa SOLO `pricing`, así que devolvemos el pricing efectivo (el pin de modo no aplica acá).
  return { pricing: resolved.pricing, modePin: resolved.mode };
}
