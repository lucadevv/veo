 ...
 * (guard de carrera en TripsService.activateScheduledTrip: solo activa si SIGUE SCHEDULED), por lo
...
export class ScheduledTripsScheduler {
  ...
  async tick(): Promise<void> {
    ...
    try {
      ...
      const ids = await this.trips.findDueScheduled(dueBefore, MAX_PER_TICK);
      if (ids.length === 0) return;
      this.logger.log(`activando ${ids.length} viaje(s) programado(s)`);
      for (const id of ids) {
        try {
          await this.trips.activateScheduledTrip(id);
        } catch (err) {
