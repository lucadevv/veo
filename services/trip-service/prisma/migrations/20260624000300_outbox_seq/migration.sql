...
CREATE SEQUENCE "trip"."outbox_events_seq_seq";
ALTER TABLE "trip"."outbox_events" ADD COLUMN "seq" BIGINT;
ALTER TABLE "trip"."outbox_events" ALTER COLUMN "seq" SET DEFAULT nextval('"trip"."outbox_events_seq_seq"');
ALTER SEQUENCE "trip"."outbox_events_seq_seq" OWNED BY "trip"."outbox_events"."seq";
...
UPDATE "trip"."outbox_events" AS o
SET "seq" = s.rn
FROM (
  SELECT "id", nextval('"trip"."outbox_events_seq_seq"') AS rn
  FROM "trip"."outbox_events"
  WHERE "seq" IS NULL
  ORDER BY "created_at" ASC, "id" ASC
) AS s
WHERE o."id" = s."id";
ALTER TABLE "trip"."outbox_events" ALTER COLUMN "seq" SET NOT NULL;
