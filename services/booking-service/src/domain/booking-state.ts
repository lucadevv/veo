 ...
 *   COBRO_PENDIENTE ──[payment.captured]──► CONFIRMADO   (txn atómica §6: lock + decremento de asiento — F3c · CONSTRUIDO)
 *   COBRO_PENDIENTE ──[payment.failed perm. / asiento-lleno]──► CANCELADO   (Refund lo hará F3c-payment — F3c · CONSTRUIDO)
 ...
 *
 * ESTADO AS-BUILT (F3c · CONSTRUIDO): el consumer de payment.captured/payment.failed que dispara
 * COBRO_PENDIENTE→CONFIRMADO/CANCELADO YA EXISTE (BookingPaymentConsumer + confirmCapture/handlePaymentFailed)
 * y el seat-lock atómico del §6 (confirmAndLockSeats, con FOR UPDATE y decremento de asiento) también. Estas
 * transiciones SÍ se ejercitan en runtime hoy. Lo que aún es PENDIENTE es F3c-payment (el Refund del lado de
 * payment-service ante razon=ASIENTO_LLENO) y el camino EN_RUTA clock-driven (F4) — no la legalidad ni el
 * disparo de COBRO_PENDIENTE→CONFIRMADO/CANCELADO, que ya están construidos.
 */
