...
CREATE SEQUENCE "booking"."outbox_events_seq_seq";
ALTER TABLE "booking"."outbox_events" ADD COLUMN "seq" BIGINT;
ALTER TABLE "booking"."outbox_events" ALTER COLUMN "seq" SET DEFAULT nextval('"booking"."outbox_events_seq_seq"');
ALTER SEQUENCE "booking"."outbox_events_seq_seq" OWNED BY "booking"."outbox_events"."seq";
...
UPDATE "booking"."outbox_events" AS o
SET "seq" = s.rn
FROM (
  SELECT "id", nextval('"booking"."outbox_events_seq_seq"') AS rn
  FROM "booking"."outbox_events"
  WHERE "seq" IS NULL
  ORDER BY "created_at" ASC, "id" ASC
) AS s
WHERE o."id" = s."id";
ALTER TABLE "booking"."outbox_events" ALTER COLUMN "seq" SET NOT NULL;
