...
async function bootstrap(): Promise<void> {
  ...
  const config = new DocumentBuilder()
    ...
    .setDescription('Contactos de confianza (OTP, BR-I06), enlaces de seguimiento firmados y página familia · VEO')
    .setVersion('0.1.0')
    .addBearerAuth()
    .build();
  ...
  logger.info(`share-service escuchando en :${port} (gRPC en ${process.env.GRPC_URL ?? '0.0.0.0:50061'})`);
}
