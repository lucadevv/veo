...
CREATE SEQUENCE "chat"."outbox_events_seq_seq";
ALTER TABLE "chat"."outbox_events" ADD COLUMN "seq" BIGINT;
ALTER TABLE "chat"."outbox_events" ALTER COLUMN "seq" SET DEFAULT nextval('"chat"."outbox_events_seq_seq"');
ALTER SEQUENCE "chat"."outbox_events_seq_seq" OWNED BY "chat"."outbox_events"."seq";
...
UPDATE "chat"."outbox_events" AS o
SET "seq" = s.rn
FROM (
  SELECT "id", nextval('"chat"."outbox_events_seq_seq"') AS rn
  FROM "chat"."outbox_events"
  WHERE "seq" IS NULL
  ORDER BY "created_at" ASC, "id" ASC
) AS s
WHERE o."id" = s."id";
ALTER TABLE "chat"."outbox_events" ALTER COLUMN "seq" SET NOT NULL;
