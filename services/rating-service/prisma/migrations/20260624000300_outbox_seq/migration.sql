...
CREATE SEQUENCE "rating"."outbox_events_seq_seq";
ALTER TABLE "rating"."outbox_events" ADD COLUMN "seq" BIGINT;
ALTER TABLE "rating"."outbox_events" ALTER COLUMN "seq" SET DEFAULT nextval('"rating"."outbox_events_seq_seq"');
ALTER SEQUENCE "rating"."outbox_events_seq_seq" OWNED BY "rating"."outbox_events"."seq";
...
UPDATE "rating"."outbox_events" AS o
SET "seq" = s.rn
FROM (
  SELECT "id", nextval('"rating"."outbox_events_seq_seq"') AS rn
  FROM "rating"."outbox_events"
  WHERE "seq" IS NULL
  ORDER BY "created_at" ASC, "id" ASC
) AS s
WHERE o."id" = s."id";
ALTER TABLE "rating"."outbox_events" ALTER COLUMN "seq" SET NOT NULL;
