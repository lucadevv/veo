 ...
 *  - Solo pagos PENDING con `externalUid` creados en la última hora (ventana configurable).
...
export class PaymentPollService implements OnModuleInit, OnModuleDestroy {
  ...
  /**
   * Un barrido: toma hasta `batch` pagos PENDING con externalUid de la última hora, consulta el estado
   * al proveedor y aplica el desenlace por el camino idempotente del webhook. Devuelve el conteo
   * (para tests/observabilidad).
   */
