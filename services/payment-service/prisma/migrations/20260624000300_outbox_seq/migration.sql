...
CREATE SEQUENCE "payment"."outbox_events_seq_seq";
ALTER TABLE "payment"."outbox_events" ADD COLUMN "seq" BIGINT;
ALTER TABLE "payment"."outbox_events" ALTER COLUMN "seq" SET DEFAULT nextval('"payment"."outbox_events_seq_seq"');
ALTER SEQUENCE "payment"."outbox_events_seq_seq" OWNED BY "payment"."outbox_events"."seq";
...
UPDATE "payment"."outbox_events" AS o
SET "seq" = s.rn
FROM (
  SELECT "id", nextval('"payment"."outbox_events_seq_seq"') AS rn
  FROM "payment"."outbox_events"
  WHERE "seq" IS NULL
  ORDER BY "created_at" ASC, "id" ASC
) AS s
WHERE o."id" = s."id";
ALTER TABLE "payment"."outbox_events" ALTER COLUMN "seq" SET NOT NULL;
