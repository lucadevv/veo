...
CREATE SEQUENCE "identity"."outbox_events_seq_seq";
ALTER TABLE "identity"."outbox_events" ADD COLUMN "seq" BIGINT;
ALTER TABLE "identity"."outbox_events" ALTER COLUMN "seq" SET DEFAULT nextval('"identity"."outbox_events_seq_seq"');
ALTER SEQUENCE "identity"."outbox_events_seq_seq" OWNED BY "identity"."outbox_events"."seq";
...
UPDATE "identity"."outbox_events" AS o
SET "seq" = s.rn
FROM (
  SELECT "id", nextval('"identity"."outbox_events_seq_seq"') AS rn
  FROM "identity"."outbox_events"
  WHERE "seq" IS NULL
  ORDER BY "created_at" ASC, "id" ASC
) AS s
WHERE o."id" = s."id";
ALTER TABLE "identity"."outbox_events" ALTER COLUMN "seq" SET NOT NULL;
