...
/**
 * Stub del PoliciesService (PBAC, ADR-024): el masking del DNI del conductor lee `pii.mask`.dniTail.
 * Devuelve el default del catálogo (4) → los asserts de enrolamiento esperan tail-4 ('****5678').
 */
...
describe('DriversService.verifyBiometric · minteo de sessionRef (BR-I02)', () => {
  it('mintea un sessionRef de un solo uso al verificar', async () => {
    ...
    const svc = new DriversService(new DriversRepository(makePrisma(okDriver) as never),
      redis as never,
      bio,
      sessions,
      config,
  ...
  it('rechaza si el conductor no está enrolado biométricamente', async () => {
    const svc = new DriversService(new DriversRepository(makePrisma({ ...okDriver, faceEmbedding: [] }) as never),
      makeRedis() as never,
      bio,
      sessions,
      config,
  ...
  it('cuando la verificación NO pasa: NO mintea sessionRef, AUDITA el intento, cuenta el lockout y lanza UnauthorizedError (A1)', async () => {
    ...
    const redis = makeRedis();
    const svc = new DriversService(new DriversRepository(prisma as never), redis as never, bioFail, sessions, config, policies);
    await expect(
      svc.verifyBiometric('u1', { challengeId: 'c1', frames: ['f1'] }),
...
describe('DriversService.enrollFace · enrolamiento KYC con liveness PASIVO (PAD anti-spoofing)', () => {
  ...
  it('sin rostro (enrollPassive sin embedding) → 422 (UnprocessableEntityError) y NO escribe ni audita', async () => {
    const prisma = makeEnrollPrisma(okDriver);
    const svc = new DriversService(new DriversRepository(prisma as never),
      makeRedis() as never,
      bioNoFace,
      sessions,
      config,
  ...
  it('404 si el conductor no existe', async () => {
    const svc = new DriversService(new DriversRepository(makeEnrollPrisma(null) as never),
      makeRedis() as never,
      bio,
      sessions,
      config,
...
describe('DriversService.startShift · gate biométrico (BR-I02)', () => {
  ...
  it('RESUME de pausa: un conductor ON_BREAK vuelve al pool por el gate biométrico (ON_BREAK→AVAILABLE)', async () => {
    ...
    const svc = new DriversService(new DriversRepository(makePrisma(onBreak, onBreak) as never),
      makeRedis({ sessions: session('ok') }) as never,
      bio,
      sessions,
      config,
  ...
  it('DOUBLE-DISPATCH cerrado: un conductor EN VIAJE (ON_TRIP) NO re-entra al pool por startShift → ConflictError', async () => {
    ...
    const svc = new DriversService(new DriversRepository(makePrisma(onTrip, onTrip) as never),
      makeRedis({ sessions: session('ok') }) as never,
      bio,
      sessions,
      config,
  ...
  it('guard DEFENSIVO: una sesión biométrica corrupta (no-pasó) → UnauthorizedError SIN auditar NI contar (verify es el único dueño del lockout)', async () => {
    ...
    const svc = new DriversService(new DriversRepository(prisma as never), redis as never, bio, sessions, config, policies);
  ...
  it('no permite turno si el KYC no está aprobado', async () => {
    const svc = new DriversService(new DriversRepository(makePrisma({ ...okDriver, backgroundCheckStatus: 'PENDING' }) as never),
      makeRedis({ sessions: session('ok') }) as never,
      bio,
      sessions,
      config,
  ...
  it('rechaza conductor suspendido', async () => {
    const svc = new DriversService(new DriversRepository(makePrisma({ ...okDriver, suspendedAt: new Date() }) as never),
      makeRedis({ sessions: session('ok') }) as never,
      bio,
      sessions,
      config,
...
describe('DriversService.suspend · suspensión MANUAL por operador (hold DISCIPLINARY)', () => {
  ...
  it('Lote 1b: una transición NUEVA a suspendido revoca TODAS las sesiones por userId (NO por Driver.id)', async () => {
    ...
    const svc = new DriversService(new DriversRepository(prisma as never), makeRedis() as never, bio, spySessions, config, policies);
...
describe('DriversService · BACKSTOP durable del revoke en las 4 vías EVENT-DRIVEN (des-gateado de created)', () => {
  ...
  it('suspendByFleet (created=true): resella por userId al epoch del suspendedAt + fast-path (revoke)', async () => {
    ...
    const s = spy();
    const svc = new DriversService(new DriversRepository(prisma as never), makeRedis() as never, bio, s as never, config, policies);
...
describe('DriversService.updatePersonalInfo · datos personales (BR-I04)', () => {
  ...
  it('exactly-once: SIN fila previa emite driver.registered (una vez); con fila previa NO re-emite', async () => {
    ...
    const svcA = new DriversService(new DriversRepository(first.prisma as never),
      makeRedis() as never,
      bio,
      sessions,
      config,
    ...
    const svcB = new DriversService(new DriversRepository(second.prisma as never),
      makeRedis() as never,
      bio,
      sessions,
      config,
...
describe('DriversService · techo de abuso del enrol + destrabe de central (F3)', () => {
  ...
  it('GATE DE ABUSO: con el cooldown lleno (5 spoofs) → 403 ANTES de gastar el PAD', async () => {
    ...
    const svc = new DriversService(new DriversRepository(makeEnrollPrismaLocal(okDriver) as never),
      redis as never,
      bioSpoof,
      sessions,
      config,
  ...
  it('clearBiometricLockout (central) borra AMBOS bloqueos: turno + enrol', async () => {
    ...
    const svc = new DriversService(new DriversRepository(makeEnrollPrismaLocal({ id: 'd1' }) as never),
      redis as never,
      bio,
      sessions,
      config,
  ...
  it('clearBiometricLockout → 404 si el conductor no existe', async () => {
    const svc = new DriversService(new DriversRepository(makeEnrollPrismaLocal(null) as never),
