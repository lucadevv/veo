/**
 * OutboxRelay — drena la tabla outbox de media y publica a Kafka (FOUNDATION §6).
...
export class OutboxRelay implements OnModuleInit, OnModuleDestroy {
  ...
  constructor(prisma: PrismaService, config: ConfigService<Env, true>) {
    ...
    // El store de outbox sobre el write client (la escritura de dominio pobló el outbox en la misma tx).
    this.store = new PrismaOutboxStore(prisma.write, 'media');
  }
