...
CREATE SEQUENCE "audit"."outbox_events_seq_seq";
ALTER TABLE "audit"."outbox_events" ADD COLUMN "seq" BIGINT;
ALTER TABLE "audit"."outbox_events" ALTER COLUMN "seq" SET DEFAULT nextval('"audit"."outbox_events_seq_seq"');
ALTER SEQUENCE "audit"."outbox_events_seq_seq" OWNED BY "audit"."outbox_events"."seq";
...
UPDATE "audit"."outbox_events" AS o
SET "seq" = s.rn
FROM (
  SELECT "id", nextval('"audit"."outbox_events_seq_seq"') AS rn
  FROM "audit"."outbox_events"
  WHERE "seq" IS NULL
  ORDER BY "created_at" ASC, "id" ASC
) AS s
WHERE o."id" = s."id";
ALTER TABLE "audit"."outbox_events" ALTER COLUMN "seq" SET NOT NULL;
