...
CREATE SEQUENCE "panic"."outbox_events_seq_seq";
ALTER TABLE "panic"."outbox_events" ADD COLUMN "seq" BIGINT;
ALTER TABLE "panic"."outbox_events" ALTER COLUMN "seq" SET DEFAULT nextval('"panic"."outbox_events_seq_seq"');
ALTER SEQUENCE "panic"."outbox_events_seq_seq" OWNED BY "panic"."outbox_events"."seq";
...
UPDATE "panic"."outbox_events" AS o
SET "seq" = s.rn
FROM (
  SELECT "id", nextval('"panic"."outbox_events_seq_seq"') AS rn
  FROM "panic"."outbox_events"
  WHERE "seq" IS NULL
  ORDER BY "created_at" ASC, "id" ASC
) AS s
WHERE o."id" = s."id";
ALTER TABLE "panic"."outbox_events" ALTER COLUMN "seq" SET NOT NULL;
