...
import { DISPATCH_REPO, type DispatchRepository } from './dispatch.repository';
...
export class DispatchService {
  ...
  constructor(
    @Inject(DISPATCH_REPO) private readonly repo: DispatchRepository,
  ...
  async accept(matchId: string, driverId: string): Promise<MatchView> {
    ...
    const pre = await this.repo.findMatchById(matchId);
    ...
    const view = await this.repo.runInTx(async (tx) => {
  ...
  private async retractSiblingOffers(tripId: string, winnerMatchId: string): Promise<void> {
    const siblings = await this.repo.findLiveSiblingOffers(tripId, winnerMatchId);
  ...
  async excludeDriverForPanic(tripId: string): Promise<string | null> {
    const match = await this.repo.findAcceptedMatchForTrip(tripId);
