/**
 * OutboxRelay — drena la tabla outbox de notification y publica a Kafka (FOUNDATION §6).
 * Bucle cada 500ms. Idempotente (republicar es seguro: dedupKey + clave por entidad).
 * Publica notification.delivered / notification.failed encolados por el motor en la misma tx.
 */
...
export class OutboxRelay implements OnModuleInit, OnModuleDestroy {
  ...
  constructor(prisma: PrismaService, config: ConfigService<Env, true>) {
    ...
    this.store = new PrismaOutboxStore(prisma.write, 'notification');
  }
