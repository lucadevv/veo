...
- **C3 · Rounding + double-source de la FÓRMULA. 🚧 Lote 1 HECHO.** La raíz de P-C: había DOS calculadoras de tarifa fija (el quote en `public-bff/maps/fare.ts` y el cobro firme en `trip-service/domain/fare.ts`) que divergían — el quote redondeaba a S/0.10 y OMITÍA surge/niño; el firme redondeaba al céntimo e incluía surge. El pasajero veía un número y se le cobraba otro.
  ...
  - **⚠️ CORRECCIÓN (2026-07-04 · auditoría /auditar contra el código real):** el "Lote 1 (hecho)" de arriba estaba SOBRE-DECLARADO — se escribió como plan, no como estado. Verificado contra develop: **(a) `computeFixedFareCents` NO EXISTÍA** en ningún branch (`git log --all -S` solo matcheaba el propio ADR); el double-source quote↔cobro seguía VIVO. **(c) el quote NO aplicaba surge** (`fare.ts` decía literal "Sin surge"), mientras el create SÍ lo aplica server-authoritative (`public-bff/trips.service.ts` `dispatch.getSurge`, Fase C) → el sobrecobro silencioso surge estaba ABIERTO, no cerrado.
    - **Cerrado de verdad (2026-07-04, este lote):** el ítem **(c)** — el QUOTE ahora resuelve el surge con el MISMO `dispatch.getSurge(user, origin)` que el create (`maps.service.fetchSurge`, solo FIXED, PUJA→1.0, fail-safe 1.0) y lo aplica en `categoryFareCents`/`V2` espejando el `× surge` del cobro firme. Límites `MIN_SURGE`/`MAX_SURGE` unificados en `@veo/shared-types` (fuente única, consumida por trip-service, dispatch-ready y el BFF). El sobrecobro SILENCIOSO surge queda cerrado.
    - **SIGUE ABIERTO → RC2 (P-C real):** el ítem **(a)** — la fuente única `computeFixedFareCents` no existe; el quote redondea a S/0.10 y el cobro firme al céntimo (divergencia ≤ S/0.10, documentada como decisión OPUESTA en `fixed-dispatch.strategy.ts`). Unificar la fórmula (con la decisión de dueño S/0.10-vs-céntimo del punto (1A)) es un lote aparte. Y **C1** (`quotedFareCents` + reconciliación) tampoco arrancó.
