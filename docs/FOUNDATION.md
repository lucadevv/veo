...
**DNI (distinción por entidad — decisión del dueño, ver VEO_SPEC_CONDUCTOR):** el **`User`** (pasajero) **NO guarda hash de DNI** — `User.dniHash` se DROPEÓ (TASK-008): era campo muerto (ningún flujo lo escribía; el KYC del pasajero es liveness+faceEmbedding) y data-minimization Ley 29733 manda. El doc de identidad del pasajero es **`User.document`** (doc de pago · Yape On File/ProntoPaga un-tap, patrón PedidosYa): **en claro POR DECISIÓN** (owner-only por JWT, la UI enmascara) y se **anula en el tombstone** (BR-S06). DNI del **`Driver`** = **cifrado reversible AES-256-GCM en reposo** (nunca en claro persistido) + **blind index `dni_hash`** (@unique, dedup), **descifrado en el borde solo para Compliance+** (admin-rail) — compliance debe MOSTRARLO para verificar manualmente contra el documento, y un hash rompería esa verificación.
