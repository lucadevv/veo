...
import { approveDriverByUserId, clearDispatchHotIndex, injectOtp, getTripPricingRow } from '../lib/fixtures.js';
import {
  getModeSchedule,
  putModeSchedule,
  resolveMode,
...
(gate.ready ? describe : describe.skip)('VEO · PUJA↔FIXED por schedule E2E (cross-servicio orquestado)', () => {
  ...
  async function setupDriver(phone: string): Promise<ReadyDriver> {
    ...
    const tok = await client.post<AuthTokens>('/auth/otp/verify', { phone, code: OTP_CODE, type: 'DRIVER' });
