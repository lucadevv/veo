 ...
 * jest.fn() vacíos. Así podemos verificar que abrir con la clave del Keychain LEE/ESCRIBE datos, con
...
const BOOTSTRAP_LITERAL = 'veo-passenger-bootstrap-v1';
...
describe('almacén seguro (passenger) — apertura directa con la clave del Keychain', () => {
  beforeEach(() => {
    jest.resetModules();
    jest.clearAllMocks();
  });
  ...
  it('arranques siguientes: reutiliza la clave del Keychain SIN re-escribir y abre con ELLA', async () => {
    const {getMock, setMock} = keychain();
  ...
  it('tras init con Keychain, secureStore LEE/ESCRIBE datos (roundtrip sobre la instancia abierta)', async () => {
    ...
    secureStore.setString(ACCESS_TOKEN_KEY, 'jwt-123');
    expect(secureStore.getString(ACCESS_TOKEN_KEY)).toBe('jwt-123');
    // La escritura llegó a la instancia MMKV segura (no a otra).
    expect(secureInstance()?.data.get(ACCESS_TOKEN_KEY)).toBe('jwt-123');
  });
  ...
  it('fallback EFÍMERO: si el Keychain falla tras los reintentos, degrada a memoria y devuelve false', async () => {
    const {getMock} = keychain();
    getMock.mockRejectedValue(new Error('keychain boom'));
    ...
    expect(secureInstance()).toBeUndefined();
    secureStore.setString(ACCESS_TOKEN_KEY, 'ephemeral');
    expect(secureStore.getString(ACCESS_TOKEN_KEY)).toBe('ephemeral');
    warn.mockRestore();
  });

  it('reintenta el Keychain ante fallos transitorios y se recupera (2 fallos → éxito)', async () => {
    const {getMock} = keychain();
    getMock
...
describe('getOrCreateEncryptionKey — entropía y persistencia', () => {
  ...
  it('lanza KeychainUnavailableError tras agotar los reintentos', async () => {
    const {getMock} = keychain();
