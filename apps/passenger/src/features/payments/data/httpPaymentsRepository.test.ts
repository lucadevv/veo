import { ApiError, type DebtView, type HttpClient, type PaymentView } from '@veo/api-client';
...
const DEBT_VIEW: DebtView = {
  ...
  debts: [
    ...
    { paymentId: 'pay-2', tripId: 'trip-2', amountCents: 800, reason: 'DECLINED', createdAt: '2026-06-03T12:00:00.000Z', kind: 'DEBT' },
    // Pago por completar (PENDING con checkout vivo): viaja en la lista pero NO suma a totalCents/hasDebt.
    { paymentId: 'pay-3', tripId: 'trip-3', amountCents: 3600, reason: '', createdAt: '2026-06-04T09:00:00.000Z', kind: 'PENDING_ACTION' },
  ],
};

const CAPTURED_PAYMENT = { id: 'pay-1', status: 'CAPTURED' } as unknown as PaymentView;
...
describe('HttpPaymentsRepository · deuda (BR-P02)', () => {
  it('getMyDebts pega a GET /payments/debts con el schema de deuda', async () => {
    ...
    await expect(repo.getMyDebts()).resolves.toMatchObject({ hasDebt: true, totalCents: 2300 });
  ...
  it('retryCharge pega a POST /payments/:id/retry-charge SIN body (el bff deriva la idempotencia)', async () => {
    ...
    await expect(repo.retryCharge('pay-1')).resolves.toMatchObject({ status: 'CAPTURED' });
    expect(post).toHaveBeenCalledWith(
      '/payments/pay-1/retry-charge',
      expect.objectContaining({ schema: expect.anything() }),
    );
    // El cobro es canónico (un Payment por viaje): NO mandamos dedupKey propia (la deriva el bff).
    const opts = post.mock.calls[0][1] as { body?: unknown; idempotencyKey?: unknown };
  ...
  it('retryCharge propaga el 404 anti-IDOR sin envolverlo (cobro ajeno/inexistente)', async () => {
    ...
    const repo = new HttpPaymentsRepository(makeHttp({ post: jest.fn().mockRejectedValue(boom) }));
  ...
  it('getPayment (poll del checkout) pega a GET /payments/:id', async () => {
    ...
    await expect(repo.getPayment('pay-1')).resolves.toMatchObject({ id: 'pay-1' });
    expect(get).toHaveBeenCalledWith(
      '/payments/pay-1',
      expect.objectContaining({ schema: expect.anything() }),
    );
...
describe('HttpPaymentsRepository · changePaymentMethod (TASK 3)', () => {
  const NEW_CHECKOUT = { id: 'pay-1', status: 'PENDING', method: 'PLIN' } as unknown as PaymentView;
  ...
  it('pega a POST /payments/:id/method con body { method } y devuelve el checkout nuevo', async () => {
    ...
    await expect(repo.changePaymentMethod('pay-1', 'PLIN')).resolves.toMatchObject({
      method: 'PLIN',
    });
    expect(post).toHaveBeenCalledWith(
      '/payments/pay-1/method',
      expect.objectContaining({ body: { method: 'PLIN' }, schema: expect.anything() }),
    );
  });

  it('422 → PaymentMethodNotApplicableError (el método no aplica: CASH)', async () => {
    const post = jest.fn().mockRejectedValue(new ApiError(422, 'CASH_NOT_ALLOWED', 'no aplica'));
    const repo = new HttpPaymentsRepository(makeHttp({ post }));

    await expect(repo.changePaymentMethod('pay-1', 'YAPE')).rejects.toBeInstanceOf(
      PaymentMethodNotApplicableError,
    );
  });

  it('409 → PaymentNotChangeableError (el pago ya no es cambiable)', async () => {
    const post = jest.fn().mockRejectedValue(new ApiError(409, 'NOT_CHANGEABLE', 'ya capturó'));
    const repo = new HttpPaymentsRepository(makeHttp({ post }));

    await expect(repo.changePaymentMethod('pay-1', 'CARD')).rejects.toBeInstanceOf(
      PaymentNotChangeableError,
    );
  });
  ...
  it('otros errores (404 anti-IDOR) se propagan sin envolver', async () => {
    ...
    await expect(repo.changePaymentMethod('pay-ajeno', 'YAPE')).rejects.toBe(boom);
  });
});
