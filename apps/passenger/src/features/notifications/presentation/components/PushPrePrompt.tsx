 ...
 */
export function PushPrePrompt({ visible, onDismiss, onEnable }: PushPrePromptProps): React.JSX.Element {
  const theme = useTheme();
  const { t } = useTranslation();

  return (
    <BottomSheet visible={visible} onClose={onDismiss} title={t('notifications.prePromptTitle')}>
      <View style={{ alignItems: 'center', gap: theme.spacing.md, paddingVertical: theme.spacing.sm }}>
        ...
        <View style={{ width: '100%', gap: theme.spacing.sm, marginTop: theme.spacing.sm }}>
          <Button label={t('notifications.prePromptEnable')} fullWidth size="lg" onPress={onEnable} />
          <Button label={t('notifications.prePromptDismiss')} variant="ghost" fullWidth onPress={onDismiss} />
        </View>
      </View>
    </BottomSheet>
  );
