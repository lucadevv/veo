 ...
 */
export class LocalCameraSharePreferenceRepository implements CameraSharePreferenceRepository {
  ...
  get(tripId: string): Promise<CameraSharePreference> {
    const stored = this.store.getJSON<CameraSharePreference>(
      this.keyFor(tripId),
    );
    if (!stored) {
    ...
    return Promise.resolve({
      ...
      allowedContactIds: Array.isArray(stored.allowedContactIds)
        ? stored.allowedContactIds.filter(
  ...
  save(tripId: string, preference: CameraSharePreference): Promise<void> {
    this.store.setJSON<CameraSharePreference>(this.keyFor(tripId), {
      ...
      allowedContactIds: preference.shareWithFamily
        ? preference.allowedContactIds
        : [],
    });
    return Promise.resolve();
  }
