...
import { getCabinVideoViewer, type CabinViewerState } from '../ports/cabinVideoViewer';
...
export function CameraLiveScreen(): React.JSX.Element {
  ...
  const screenState: 'connecting' | 'live' | 'error' | 'noPermission' = query.isError
    ? 'noPermission'
  ...
  return (
    <View style={[styles.root, { backgroundColor: VIDEO_BG }]}>
        ...
        <View style={styles.center} pointerEvents="none">
          {screenState === 'connecting' ? (
            <>
              ...
              <Text variant="callout" color="inkMuted" align="center" style={styles.stateText}>
      ...
      <Scrim direction="top" style={[styles.gradientTop, { height: 180 + insets.top }]} />
      ...
      <View style={[styles.topBar, { top: insets.top + theme.spacing.sm }]}>
        ...
        {hasGrant ? <StatusPill label={t('cameraLive.recLive')} tone="danger" dot live /> : null}
      </View>
      ...
      <View style={[styles.bottom, { paddingBottom: insets.bottom + theme.spacing.xl }]}>
        <View
          style={[
            styles.card,
            { backgroundColor: theme.colors.surfaceElevated, borderRadius: theme.radii.lg, borderColor: theme.colors.border },
          ]}
        >
          <View style={styles.cardHeader}>
            ...
            {isLive ? <StatusPill label={t('cameraLive.analyzing')} tone="success" dot /> : null}
          </View>

          {isLive ? (
            <View style={[styles.viewingRow, { borderTopColor: theme.colors.border }]}>
              <Text variant="footnote" color="inkMuted">
  ...
  return (
    <View style={style} pointerEvents="none">
      <Svg width="100%" height="100%">
        <Defs>
          <SvgLinearGradient id={`scrim-${direction}`} x1="0" y1="0" x2="0" y2="1">
            <Stop offset="0" stopColor={VIDEO_BG} stopOpacity={topOpacity} />
            <Stop offset="1" stopColor={VIDEO_BG} stopOpacity={bottomOpacity} />
          </SvgLinearGradient>
        </Defs>
        <Rect x="0" y="0" width="100%" height="100%" fill={`url(#scrim-${direction})`} />
      </Svg>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  bottom: { position: 'absolute', left: 0, right: 0, bottom: 0, paddingHorizontal: 22, gap: 12 },
