 ...
 */
export function RecentTripRow({ trip, onSelect }: RecentTripRowProps): React.JSX.Element | null {
  ...
  const meta = useMemo(() => {
    ...
    const duration = t('history.minutes', { minutes: formatDurationMinutes(trip.durationSeconds) });
  ...
  return (
    <Animated.View style={animatedStyle}>
      <Pressable
        ...
        accessibilityLabel={t('home.recentTripRowLabel', { destination: label.title, meta })}
        onPress={() =>
          onSelect({ point: { lat: label.lat, lng: label.lng }, title: label.title, subtitle: label.subtitle })
        }
        onPressIn={onPressIn}
...
const styles = StyleSheet.create({
  ...
  iconWrap: { width: ICON, height: ICON, alignItems: 'center', justifyContent: 'center' },
  labels: { flex: 1, gap: 2 },
});
