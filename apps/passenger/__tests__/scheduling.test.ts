...
describe('validateScheduledFor · ventana [≥15min, ≤7días]', () => {
  ...
  it('rechaza por debajo del mínimo de 15 minutos (TOO_SOON)', () => {
    ...
    expect(validateScheduledFor(target, NOW)).toEqual({ valid: false, reason: 'TOO_SOON' });
  });
  ...
  it('rechaza más allá de 7 días (TOO_FAR)', () => {
    const target = new Date(NOW.getTime() + MAX_SCHEDULE_HORIZON_MS + 60 * 1000);
    expect(validateScheduledFor(target, NOW)).toEqual({ valid: false, reason: 'TOO_FAR' });
  });
