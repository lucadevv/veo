...
import { RemoveAvatarUseCase, UploadAvatarUseCase } from '../src/features/profile/domain/usecases';
...
describe('HttpAvatarUploader', () => {
  it('pide el ticket, sube el binario con PUT, confirma y devuelve la publicUrl SELLADA', async () => {
    ...
    const uploader = new HttpAvatarUploader(makeHttp(post), fetchImpl as unknown as typeof fetch);
  ...
  it('lanza AvatarUploadError(too-large) y NO sube ni confirma si la imagen excede maxBytes', async () => {
    ...
    await expect(uploader.uploadAvatar(jpegFile)).rejects.toMatchObject({ reason: 'too-large' });
    // Solo leyó el binario local; nunca intentó el PUT a MinIO.
    expect(fetchImpl).toHaveBeenCalledTimes(1);
    // Solo pidió el presign; nunca confirmó.
    expect(post).toHaveBeenCalledTimes(1);
    expect(post).toHaveBeenCalledWith('/users/me/avatar/presign', expect.anything());
  });
  ...
  it('lanza AvatarUploadError(confirm) si el backend rechaza la confirmación (cuota servidor)', async () => {
    ...
    await expect(uploader.uploadAvatar(jpegFile)).rejects.toMatchObject({ reason: 'confirm' });
  ...
  it('deriva el descriptor por extensión cuando no hay MIME (png)', async () => {
    ...
    await uploader.uploadAvatar({ ...jpegFile, mimeType: null, fileName: 'foto.PNG' });
  ...
  it('lanza AvatarUploadError(unsupported-type) si el formato no está en la lista blanca', async () => {
    ...
    await expect(
      uploader.uploadAvatar({ ...jpegFile, mimeType: 'image/gif', fileName: 'foto.gif', uri: 'file:///tmp/foto.gif' }),
  ...
  it('lanza AvatarUploadError(presign) si el BFF falla al expedir el ticket', async () => {
    ...
    await expect(uploader.uploadAvatar(jpegFile)).rejects.toBeInstanceOf(AvatarUploadError);
    await expect(uploader.uploadAvatar(jpegFile)).rejects.toMatchObject({ reason: 'presign' });
  });
  ...
  it('lanza AvatarUploadError(upload) si MinIO responde un status != 2xx', async () => {
    ...
    const uploader = new HttpAvatarUploader(
      makeHttp(post),
      makeFetch({ uploadOk: false, uploadStatus: 403 }) as unknown as typeof fetch,
    );

    await expect(uploader.uploadAvatar(jpegFile)).rejects.toMatchObject({ reason: 'upload' });
  });
  ...
  it('lanza AvatarUploadError(network) si el PUT del binario falla por red', async () => {
    ...
    const fetchImpl = jest.fn(async (_input: RequestInfo | URL, init?: RequestInit) => {
      if (!init) {
        return { blob: async () => ({ size: 10 }) as Blob } as unknown as Response;
      }
      throw new Error('network down');
    });
    const uploader = new HttpAvatarUploader(makeHttp(post), fetchImpl as unknown as typeof fetch);

    await expect(uploader.uploadAvatar(jpegFile)).rejects.toMatchObject({ reason: 'network' });
  });
  ...
  it('lanza AvatarUploadError(read) si no se puede abrir el archivo local (content:// frágil)', async () => {
    ...
    await expect(
      uploader.uploadAvatar({ ...jpegFile, uri: 'content://media/external/images/1' }),
  ...
  it('lanza AvatarUploadError(read) si la respuesta local no soporta blob()', async () => {
    ...
    await expect(uploader.uploadAvatar(jpegFile)).rejects.toMatchObject({ reason: 'read' });
  });
...
describe('UploadAvatarUseCase', () => {
  ...
  it('propaga el error del uploader sin tocar el repositorio', async () => {
    ...
    await expect(useCase.execute(jpegFile)).rejects.toMatchObject({ reason: 'upload' });
