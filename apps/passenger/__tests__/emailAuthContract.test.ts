...
import { isValidEmail, isValidPassword } from '../src/features/auth/presentation/hooks/useEmailAuthFlow';
...
describe('Contrato auth por correo (@veo/api-client)', () => {
  it('emailRegister exige password ≥ 12 y type válido; name es opcional', () => {
    expect(
      emailRegister.safeParse({ email: 'ana@veo.pe', password: 'x'.repeat(12), type: 'PASSENGER' })
        .success,
    ).toBe(true);
    expect(
      emailRegister.safeParse({ email: 'ana@veo.pe', password: 'corta', type: 'PASSENGER' }).success,
    ).toBe(false);
    expect(
      emailRegister.safeParse({ email: 'no-es-correo', password: 'x'.repeat(12), type: 'PASSENGER' })
        .success,
    ).toBe(false);
  });

  it('emailVerify y emailReset exigen código de 6 caracteres', () => {
    expect(emailVerify.safeParse({ email: 'ana@veo.pe', code: '123456' }).success).toBe(true);
    expect(emailVerify.safeParse({ email: 'ana@veo.pe', code: '123' }).success).toBe(false);
    expect(
      emailReset.safeParse({ email: 'ana@veo.pe', code: '123456', newPassword: 'x'.repeat(12) })
        .success,
    ).toBe(true);
    expect(
      emailReset.safeParse({ email: 'ana@veo.pe', code: '123456', newPassword: 'corta' }).success,
    ).toBe(false);
  });

  it('emailLogin exige correo válido y password no vacío', () => {
    expect(emailLogin.safeParse({ email: 'ana@veo.pe', password: 'algo' }).success).toBe(true);
    expect(emailLogin.safeParse({ email: 'ana@veo.pe', password: '' }).success).toBe(false);
  });
