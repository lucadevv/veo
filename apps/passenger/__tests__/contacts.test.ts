...
import type { NewTrustedContact, TrustedContact } from '../src/features/contacts/domain/entities';
...
describe('AddContactUseCase', () => {
  it('normaliza el teléfono (antepone 51) y agrega cuando los campos son válidos', async () => {
    ...
    await useCase.execute({ name: 'María Pérez', phone: '9 8765 4321', relationship: 'madre' });

    expect(repo.add).toHaveBeenCalledWith(
      expect.objectContaining({ phone: '51987654321', name: 'María Pérez', relationship: 'madre' }),
    );
  });
  ...
  it('rechaza un teléfono inválido', () => {
    ...
    expect(() => useCase.execute({ name: 'María', phone: '123', relationship: 'madre' })).toThrow(
      ContactValidationError,
    );
  ...
  it('rechaza un parentesco vacío', () => {
    ...
    expect(() => useCase.execute({ name: 'María', phone: '987654321', relationship: '' })).toThrow(
      /relationship|parentesco/i,
    );
  });
});
