...
class FakeTripRepository implements TripRepository {
  ...
  acceptOffer = jest.fn(async (_id: string, _d: string): Promise<OfferView> => OFFER);
  cancelBid = jest.fn(async (_id: string): Promise<void> => undefined);
  rebid = jest.fn(async (_id: string, _b: number): Promise<TripResource> => fakeTrip());
  getSurge = jest.fn(async (_c: GeoPoint): Promise<SurgeQuote> => ({ multiplier: 1, zoneId: 'z', active: false }));
  createTrip = jest.fn(async (_i: CreateTripRequest): Promise<TripResource> => fakeTrip());
  ...
  cancelTrip = jest.fn(async (_id: string, _i: CancelTripRequest): Promise<TripResource> => fakeTrip());
  changeDestination = jest.fn(async (_id: string, _d: GeoPoint): Promise<TripResource> => fakeTrip());
  getVideoGrant = jest.fn(async (_id: string): Promise<TripVideoGrant> => ({ url: 'u', token: 't' }));
  shareTrip = jest.fn(async (_id: string, _i?: ShareTripRequest): Promise<CreatedShareLink> => ({} as CreatedShareLink));
...
describe('PUJA · HttpTripRepository (endpoints reales del BFF)', () => {
  it('listOffers → GET /trips/:id/offers', async () => {
    ...
    expect(get).toHaveBeenCalledWith('/trips/trip-1/offers', { schema: expect.anything() });
  });
