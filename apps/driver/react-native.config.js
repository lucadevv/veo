 ...
 *
 * `assets` declara las carpetas de fuentes de marca VEO (Clash Display + Outfit + Fraunces).
 * Al correr `npx react-native-asset` (o `react-native link`), la CLI copia los .ttf/.otf a
 *   - iOS: ios/VEODriver/ + registra cada archivo en UIAppFonts del Info.plist + los agrega al target Xcode.
 ...
 *   - iOS: fuentes en ios/Fonts/ + UIAppFonts ya poblado en ios/VEODriver/Info.plist. Falta SOLO agregar la
 *     carpeta ios/Fonts al target en Xcode (Build Phases → Copy Bundle Resources) — paso manual.
