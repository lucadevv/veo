...
import {InvalidOtpCodeError, InvalidPhoneError, RequestOtpUseCase, VerifyOtpUseCase} from '../index';
...
class FakeAuthRepository implements AuthRepository {
  ...
  verifyOtp(input: {phone: string; code: string; type: 'PASSENGER' | 'DRIVER'}): Promise<AuthTokens> {
...
describe('VerifyOtpUseCase', () => {
  it('rechaza códigos que no son de 6 dígitos', async () => {
    const repo = new FakeAuthRepository();
    await expect(() => new VerifyOtpUseCase(repo).execute('987654321', '123')).toThrow(InvalidOtpCodeError);
