...
import { AvatarUploadError, type AvatarUploader } from '../../domain/ports/avatar-uploader';
import type { PickedImage } from '../../../documents/domain/ports/image-picker-service';

/** Endpoint del ticket prefirmado (el driver-bff ya tiene el guard de conductor por el JWT global). */
const AVATAR_PRESIGN_PATH = '/drivers/me/avatar/presign';
/** Endpoint de confirmación: valida la cuota, sella la `publicUrl` y persiste la foto en el perfil. */
const AVATAR_CONFIRM_PATH = '/drivers/me/avatar/confirm';
...
/**
 * Implementación de `AvatarUploader` con subida prefirmada a S3/MinIO. Espejo del uploader del pasajero.
 *
 * Reparto de responsabilidades respetando que la PUT NO pasa por el cliente autenticado del BFF:
 *  - El `HttpClient` (Bearer del BFF) SOLO se usa para pedir el ticket (presign) y confirmar.
...
export class HttpAvatarUploader implements AvatarUploader {
  ...
  async uploadAvatar(file: PickedImage): Promise<{ photoUrl: string }> {
    ...
    // 2) Lee el binario local como blob (robusto en iOS/Android: `file://` y `content://`).
    const blob = await this.readLocalBlob(file.uri);

    // 2.5) Cuota de tamaño en CLIENTE (fail-fast): el presign PUT no puede acotar el Content-Length. El
    //       backend la revalida en el `confirm` (autoritativo); este chequeo es solo UX para un mensaje claro.
    ...
    // 4) Confirma con el BFF: media-service valida la cuota REAL (autoritativo, borra si excede) y el
    //    driver-bff persiste la foto en el perfil (identity `User.photoUrl`). Devuelve la `publicUrl`
    //    sellada (NO la del ticket: solo tras una confirmación válida la foto es legítima).
    try {
  ...
  /**
   * Lee el binario local de la URI elegida como `Blob`, robusto en iOS y Android. Etapas separadas para
   * dar un motivo accionable (`read`) distinto de un fallo de red.
   */
