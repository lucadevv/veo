...
/**
 * Viewer de solo recepción del habitáculo vía LiveKit self-hosted.
 * Usa exclusivamente la URL y el token entregados por el bff (no inventa credenciales).
 * Si no hay stream o falla la conexión, muestra un estado tranquilo (nunca un error crudo).
 */
export function CabinVideo({ grant }: { grant: FamilyVideoGrant }) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (disposed) return;
        // Adjunta pistas ya publicadas al conectar.
        let hasVideo = false;
  ...
  return (
    <Card className="overflow-hidden">
      <div className="flex items-center justify-between gap-2 px-4 py-3">
        <h2 className="text-sm font-medium text-ink-muted">Cámara del viaje</h2>
        {state === 'live' ? (
        ...
        {state !== 'live' ? (
          <div className="absolute inset-0 grid place-items-center p-6 text-center">
            <div className="max-w-xs text-sm text-ink-muted">
              ...
              {state === 'waiting' ? <p>La cámara se mostrará cuando esté disponible.</p> : null}
              {state === 'error' ? (
