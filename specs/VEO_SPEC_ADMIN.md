...
>
> El gate ejecutable de este principio es el test golden-rule `audit.consumer.coverage.spec.ts` (todo evento de `EVENT_SCHEMAS` está auditado o excluido con razón). **DEUDA conocida pendiente de cerrar** (única divergencia contra "todo todo"): `driver.went_offline` está excluido TEMPORALMENTE del WORM — su rama `shift_end` (el conductor cierra turno a propósito) ES una mutación deliberada que **debe auditarse**, pero hoy el evento la mezcla con `disconnect` (best-effort sin outbox, firehose-adjacent) y no tiene par `went_online`. Gatillo: separar las ramas (o agregar `went_online`) → auditar `shift_end`. Ver el marcador `DEUDA:` en `audit.consumer.ts`.
