...
>
> 🆕 **RECONCILIADO por [ADR-023](../docs/adr/023-modelo-pricing-coexistencia.md) (2026-07-07) — el MODELO DE PRICING canónico (COEXISTENCIA):** los modos de pricing son **3 PUROS** — **FIJO** (Uber: la plataforma computa la fórmula = precio), **PUJA** (inDrive: el pasajero ofrece ≥ piso, el conductor acepta/contra-oferta), **COST-SHARE** (BlaBlaCar: el conductor pone ≤ tope, ÷ asientos + service fee, no-comercial). Aclaraciones sobre ESTA matriz: (1) el eje **PROGRAMADO/AHORA es el FLUJO** (programado vs on-demand), **ORTOGONAL** al modo de pricing; (2) el "carpooling = **FIJO**" de arriba es, en la taxonomía canónica, **COST-SHARE** (conductor ≤ cost-cap, no-comercial) — NO el FIXED=Uber; (3) el modo lo asigna el admin **por servicio, a MANO** (palanca manual, sin franjas horarias — **ADR-011 superseded**); (4) **surge AFUERA** del modelo; (5) dos bordes honestos: **Mecánico = call-out plano** (visita, no viaje), **Carpooling = producto propio** (`booking-service`). Donde esta matriz y ADR-023 difieran en el modelo de pricing, **manda 023**.
